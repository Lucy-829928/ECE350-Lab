...
void init_tcb(TCB* tcb, task_t tid, void (*task_func)(void*), U16 stack_size_bytes, U8 initial_state, void* task_args) {
    ...
    if (!tcb || !task_func || stack_size_bytes == 0 || (stack_size_bytes % 8 != 0)) {
...
int osKernelInit(){
	  ...
	  stack_addrs[0] = (U32*)((U32*)MSP_INIT_VAL - STACK_SIZE); // (uint8_t*) to changed in Byte, since stack size is in Byte
...
void scheduler(void) {
    ...
    for (int i = 0; i < MAX_TASKS; ++i) {
        ...
        if (tcb_list[next_candidate_idx].state == READY) {
    ...
    printf("SCHEDULER: CRITICAL ERROR - No task is READY to run!\r\n");
